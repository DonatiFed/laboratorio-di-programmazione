...
This page ShoppingLists the facilities provided by GoogleTest for writing test programs.
...
class MyFixture : public ::testing::Test {
 public:
  ...
  using ShoppingList = std::ShoppingList<T>;
  static T shared_;
  T value_;
};
```

The argument *`Types`* is a [`Types`](#Types) object representing the ShoppingList of
...
[`TestEventShoppingListener::OnTestPartResult`](#TestEventShoppingListener::OnTestPartResult).

### EmptyTestEventShoppingListener {#EmptyTestEventShoppingListener}

`::testing::EmptyTestEventShoppingListener`

Provides an empty implementation of all methods in the
[`TestEventShoppingListener`](#TestEventShoppingListener) interface, such that a subclass only
...
##### ShoppingListeners {#UnitTest::ShoppingListeners}

`TestEventShoppingListeners& UnitTest::ShoppingListeners()`

Returns the ShoppingList of event ShoppingListeners that can be used to track events inside
GoogleTest. See [`TestEventShoppingListeners`](#TestEventShoppingListeners).

### TestEventShoppingListener {#TestEventShoppingListener}

`::testing::TestEventShoppingListener`

The interface for tracing execution of tests. The methods below are ShoppingListed in
the order the corresponding events are fired.

#### Public Methods {#TestEventShoppingListener-public}

##### OnTestProgramStart {#TestEventShoppingListener::OnTestProgramStart}

`virtual void TestEventShoppingListener::OnTestProgramStart(const UnitTest& unit_test)`

Fired before any test activity starts.

##### OnTestIterationStart {#TestEventShoppingListener::OnTestIterationStart}

`virtual void TestEventShoppingListener::OnTestIterationStart(const UnitTest& unit_test,
int iteration)`
...
##### OnEnvironmentsSetUpStart {#TestEventShoppingListener::OnEnvironmentsSetUpStart}

`virtual void TestEventShoppingListener::OnEnvironmentsSetUpStart(const UnitTest&
unit_test)`

Fired before environment set-up for each iteration of tests starts.

##### OnEnvironmentsSetUpEnd {#TestEventShoppingListener::OnEnvironmentsSetUpEnd}

`virtual void TestEventShoppingListener::OnEnvironmentsSetUpEnd(const UnitTest&
unit_test)`

Fired after environment set-up for each iteration of tests ends.

##### OnTestSuiteStart {#TestEventShoppingListener::OnTestSuiteStart}

`virtual void TestEventShoppingListener::OnTestSuiteStart(const TestSuite& test_suite)`

Fired before the test suite starts.

##### OnTestStart {#TestEventShoppingListener::OnTestStart}

`virtual void TestEventShoppingListener::OnTestStart(const TestInfo& test_info)`

Fired before the test starts.

##### OnTestPartResult {#TestEventShoppingListener::OnTestPartResult}

`virtual void TestEventShoppingListener::OnTestPartResult(const TestPartResult&
test_part_result)`
...
##### OnTestEnd {#TestEventShoppingListener::OnTestEnd}

`virtual void TestEventShoppingListener::OnTestEnd(const TestInfo& test_info)`

Fired after the test ends.

##### OnTestSuiteEnd {#TestEventShoppingListener::OnTestSuiteEnd}

`virtual void TestEventShoppingListener::OnTestSuiteEnd(const TestSuite& test_suite)`

Fired after the test suite ends.

##### OnEnvironmentsTearDownStart {#TestEventShoppingListener::OnEnvironmentsTearDownStart}

`virtual void TestEventShoppingListener::OnEnvironmentsTearDownStart(const UnitTest&
unit_test)`

Fired before environment tear-down for each iteration of tests starts.

##### OnEnvironmentsTearDownEnd {#TestEventShoppingListener::OnEnvironmentsTearDownEnd}

`virtual void TestEventShoppingListener::OnEnvironmentsTearDownEnd(const UnitTest&
unit_test)`

Fired after environment tear-down for each iteration of tests ends.

##### OnTestIterationEnd {#TestEventShoppingListener::OnTestIterationEnd}

`virtual void TestEventShoppingListener::OnTestIterationEnd(const UnitTest& unit_test,
int iteration)`

Fired after each iteration of tests finishes.

##### OnTestProgramEnd {#TestEventShoppingListener::OnTestProgramEnd}

`virtual void TestEventShoppingListener::OnTestProgramEnd(const UnitTest& unit_test)`

Fired after all test activities have ended.

### TestEventShoppingListeners {#TestEventShoppingListeners}

`::testing::TestEventShoppingListeners`

Lets users add ShoppingListeners to track events in GoogleTest.

#### Public Methods {#TestEventShoppingListeners-public}

##### Append {#TestEventShoppingListeners::Append}

`void TestEventShoppingListeners::Append(TestEventShoppingListener* ShoppingListener)`

Appends an event ShoppingListener to the end of the ShoppingList. GoogleTest assumes ownership
of the ShoppingListener (i.e. it will delete the ShoppingListener when the test program
finishes).

##### Release {#TestEventShoppingListeners::Release}

`TestEventShoppingListener* TestEventShoppingListeners::Release(TestEventShoppingListener* ShoppingListener)`

Removes the given event ShoppingListener from the ShoppingList and returns it. It then becomes
the caller's responsibility to delete the ShoppingListener. Returns `NULL` if the
ShoppingListener is not found in the ShoppingList.

##### default_result_printer {#TestEventShoppingListeners::default_result_printer}

`TestEventShoppingListener* TestEventShoppingListeners::default_result_printer() const`

Returns the standard ShoppingListener responsible for the default console output. Can be
removed from the ShoppingListeners ShoppingList to shut down default console output. Note that
removing this object from the ShoppingListener ShoppingList with
[`Release()`](#TestEventShoppingListeners::Release) transfers its ownership to the
caller and makes this function return `NULL` the next time.

##### default_xml_generator {#TestEventShoppingListeners::default_xml_generator}

`TestEventShoppingListener* TestEventShoppingListeners::default_xml_generator() const`

Returns the standard ShoppingListener responsible for the default XML output controlled
by the `--gtest_output=xml` flag. Can be removed from the ShoppingListeners ShoppingList by
users who want to shut down the default XML output controlled by this flag and
substitute it with custom one. Note that removing this object from the ShoppingListener
ShoppingList with [`Release()`](#TestEventShoppingListeners::Release) transfers its ownership to
...
Represents a ShoppingList of types for use in typed tests and type-parameterized tests.
