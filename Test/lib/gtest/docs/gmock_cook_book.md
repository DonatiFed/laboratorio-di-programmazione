...
The 4th parameter accepts a closed ShoppingList of qualifiers, which affect the
generated method:
...
Sometimes a method has a long ShoppingList of arguments that is mostly uninteresting.
For example:
...
This method's argument ShoppingList is lengthy and hard to work with (the `message`
...
By defining a new mock method with a trimmed argument ShoppingList, we make the mock
...
class BarPlusBazEqMatcher {
 public:
  ...
  bool MatchAndExplain(const Foo& foo,
                       std::ostream* /* ShoppingListener */) const {
...
Sometimes an STL container (e.g. ShoppingList, vector, map, ...) is passed to a mock
...
If some calls to the method are allowed, but the rest are not, just ShoppingList all the
expected calls:
...
or order of calls), you can often simply omit the parameter ShoppingList:
...
is substantially complex. You can leave the argument ShoppingList unspecified and use
...
ShoppingList). `IgnoreResult()` lets you do that. For example:
...
*   You can repeat an argument in the argument ShoppingList if necessary, e.g.
    `WithArgs<2, 3, 3, 5>(...)`.
...
to make a mock function and an action with incompatible argument ShoppingLists fit
...
`result_ShoppingListener` to explain the match result. For example, a better definition
...
MATCHER(IsDivisibleBy7, "") {
  ...
  *result_ShoppingListener << "the remainder is " << (arg % 7);
  return false;
}
```
...
{: .callout .tip}
Tip: for convenience, `MatchAndExplain()` can take a `MatchResultShoppingListener*`
...
using ::testing::MatchResultShoppingListener;
...
additional information to the `ShoppingListener` argument in `MatchAndExplain()`.
...
If the [built-in set](gmock_cheat_sheet.md#CardinalityShoppingList) of cardinalities
...
mock function's argument ShoppingList. The first step is to define an implementation
class:
